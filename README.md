...
This tutorial/lab aims to provide hands-on experience of building an Android app that retrieves and displays data with REST API. In the first part, we will introduce how to:
...
You will need to combine the knowledge learned from the first part and modify the provided classes/Activity to show a list of top-rated movies, as illustrated below.
...
1. To run the example project, you need an API key from The Movie DB. Follow [this instruction](https://developers.themoviedb.org/3/getting-started/introduction) to create an account on The Movie DB and get your API key.
...
3. Once the app is running, you will see a main screen displaying movie data. The "Program List" button leads you to `ProgramListActivity` that displays a list of strings with `RecyclerView`. The "Movie List" button leads you to `MovieListActivity` that you are asked to finish in this lab.
...
The fields we need in this tutorial are: `title`, `release_date`, `vote_average`, `overview`, and `poster_path`. First, we create a `Movie` class to hold the data returned by the API:
...
Next, Retrofit needs an interface class, `MovieApiService`, to bridge HTTP API:
...
Now you can generate a `Retrofit` object that generates the implementation of the `MovieApiService` in the `MainActivity`. Each `Call` from the created `MovieApiService` makes an HTTP request to the remote webserver.
...
Note that while `Call` can be executed synchronously or asynchronously, a synchronous call means that the main/UI thread will be completely blocked until the operation completes---this is not recommended and not allowed in Android. Therefore, we make an asynchronous request with `call.enqueue()`as follows:
...
* Create an Adapter class (`ProgramListAdapter.java`) that extends `RecyclerView.Adapter` to feed your data to the list. In the Adapter class, we need to:
  ...
  * Overwrite three built-in methods: 
    ...
    `onBindViewHolder()` (for binding the data of single item to a view), and 
...
* In the main Activity (`ProgramListActivity.java`), obtain a handle to the `RecyclerView` object, connect it to a layout manager, and attach the adapter for the data to be displayed
...
## Lab: Complete the Provided Classes to Display a List of Top-rated Movies

The lab asks you to display top-rated movies in `MovieListActivity`. The data can be retrieved from here:
...
Also, you will need to check `movie_row.xml` when you modify`MovieListAdapter` to know where to place the data. Note that